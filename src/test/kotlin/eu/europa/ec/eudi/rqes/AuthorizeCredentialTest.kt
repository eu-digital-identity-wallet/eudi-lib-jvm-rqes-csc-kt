...
class AuthorizeCredentialTest {
    ...
    @Test
    fun `successful credential authorization with authorization code flow without RAR`() = runTest {
        ...
        with(mockPublicClient(mockedKtorHttpClientFactory, rarUsage = RarUsage.Never)) {
            val authRequestPrepared = prepareCredentialAuthorizationRequest(credentialAuthorizationSubject).getOrThrow()
    ...
    @Test
    fun `successful credential authorization with authorization code flow with RAR`() = runTest {
        val mockedKtorHttpClientFactory = mockedKtorHttpClientFactory(
            authServerWellKnownMocker(),
            credentialsInfoPostMocker(),
            tokenPostMocker(
    ...
    private val credentialAuthorizationSubject = CredentialAuthorizationSubject(
